...
export async function GET(req:Request){
 ...
 if(p.get('type'))q.type=p.get('type');
 if(p.get('actorId'))q.actorId=p.get('actorId');
 if(p.get('breakdownId')){try{q.breakdownId=new ObjectId(p.get('breakdownId')!)}catch{return NextResponse.json({error:'Geçersiz arıza kimliği'},{status:400})}}
 const d=await db();
