...
export async function POST(req: Request) {
    ...
    const now = new Date();
    const lock = await database.collection<LoginAttemptDoc>('login_attempts').findOne({ _id: key });
    if (lock &&
    ...
    if (!valid) {
        ...
        await database.collection<LoginAttemptDoc>('login_attempts').updateOne({ _id: key }, within
            ? {
    ...
    await database.collection<LoginAttemptDoc>('login_attempts').deleteOne({ _id: key });
