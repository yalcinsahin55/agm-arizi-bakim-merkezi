...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const rl=await rateLimit(req,'breakdown:assign',u,30); if(!rl.ok)return rateLimitResponse(rl.retryAfter);
 let bid:ObjectId; try{bid=new ObjectId(id)}catch{return NextResponse.json({error:'Geçersiz arıza kimliği'},{status:400})}
 let body:unknown; try{body=await req.json()}catch{return NextResponse.json({error:'Geçersiz istek'},{status:400})}
 const technicianId=typeof body==='object'&&body!==null&&'technicianId' in body?String((body as {technicianId?:unknown}).technicianId||''):''; if(!technicianId)return NextResponse.json({error:'Teknisyen gerekli'},{status:400});
 ...
 if(b.archived)return NextResponse.json({error:'Arşivlenmiş arıza yeniden atanamaz'},{status:409});
