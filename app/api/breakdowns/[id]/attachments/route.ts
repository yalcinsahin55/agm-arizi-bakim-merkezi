...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await d.collection('breakdown_events').insertOne({ breakdownId: oid, eventId: `attachment:${ins.insertedId}`, type: 'attachment_added', actorId: u._id, actorName: u.name, note: file.name, createdAt: now });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await d.collection('breakdown_events').insertOne({ breakdownId: oid, eventId: `attachment_deleted:${aid}:${Date.now()}`, type: 'attachment_deleted', actorId: u._id, actorName: u.name, note: a.fileName, createdAt: new Date() });
