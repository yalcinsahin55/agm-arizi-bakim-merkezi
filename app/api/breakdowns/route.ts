...
export async function POST(req: Request) {
    ...
    try {
        motor = await d.collection('motors').findOne({ _id: parsed.data.motorId, active: true }) as MotorDoc | null;
...
export async function PATCH(req: Request) {
    ...
    if (p.data.motorId || p.data.categoryId || p.data.subcategoryId !== undefined) {
        ...
        try {
            if (p.data.motorId)
                motor = await d.collection('motors').findOne({ _id: String(p.data.motorId), active: true }) as MotorDoc | null;
