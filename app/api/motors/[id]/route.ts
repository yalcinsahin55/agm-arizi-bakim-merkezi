...
const equipmentTypes: EquipmentType[] = ['motor', 'booster', 'pompa', 'kompresor', 'jenerator', 'alternator', 'diger'];
...
export async function PATCH(req: Request, { params }: {
    ...
    if (body.equipmentType !== undefined && (typeof body.equipmentType !== 'string' || !equipmentTypes.includes(body.equipmentType as EquipmentType)))
    ...
    if (typeof body.name === 'string' && body.name.trim()) {
        ...
        if (duplicate) return NextResponse.json({ error: 'Aynı isimde aktif bir ekipman zaten var.' }, { status: 409 });
    ...
    if (typeof body.equipmentType === 'string' && equipmentTypes.includes(body.equipmentType as EquipmentType))
        set.equipmentType = body.equipmentType;
    if (typeof body.location === 'string')
        set.location = body.location.trim();
    ...
    await writeAudit(database, {
        ...
        fieldChanges: {
            ...
            ...(typeof body.name === 'string' && body.name.trim()
                ...
                : {}),
            ...(typeof body.hours === 'number' ? { hours: { from: motor.hours ?? motor.currentHours ?? null, to: body.hours } } : {}),
            ...(typeof body.load === 'number' ? { load: { from: motor.load ?? motor.currentLoad ?? null, to: body.load } } : {}),
            ...(typeof body.equipmentType === 'string' ? { equipmentType: { from: motor.equipmentType ?? 'motor', to: body.equipmentType } } : {}),
            ...(typeof body.location === 'string' ? { location: { from: motor.location ?? null, to: body.location.trim() } } : {}),
            ...(typeof body.description === 'string' ? { description: { from: motor.description ?? null, to: body.description.trim() } } : {}),
        },
        createdAt: new Date(),
    });
