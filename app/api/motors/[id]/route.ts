...
export async function GET(_req: Request, { params }: {
    ...
    const motor = await database.collection('motors').findOne({ _id: id, active: true });
    if (!motor)
...
export async function PATCH(req: Request, { params }: {
    ...
    const { id } = await params;
    const database = await db();
    const motor = await database.collection('motors').findOne({ _id: id });
    if (!motor)
        return NextResponse.json({ error: 'Motor bulunamadı' }, { status: 404 });
    ...
    const result = await database.collection('motors').updateOne({ _id: id }, { $set: set });
