...
export default async function Tech() {
    ...
    if (!u)
        return null;
    const rows = await (await db()).collection('breakdowns').find({ assignedTechnicianId: u._id, status: { $in: ['atandi', 'devam_ediyor', 'revizyon'] }, archived: { $ne: true } }).sort({ priority: 1, createdAt: 1 }).toArray();
