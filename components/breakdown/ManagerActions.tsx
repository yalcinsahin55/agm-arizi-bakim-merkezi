...
export default function ManagerActions({ breakdown }: {
    breakdown: Breakdown;
}) {
    ...
    async function assign() {
        ...
        if (!t)
            return;
        setBusy(true);
        const x = await fetch(`/api/breakdowns/${breakdown._id}/assign`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ technicianId: String(t._id) }) });
        setBusy(false);
        if (x.ok)
            router.refresh();
        else
