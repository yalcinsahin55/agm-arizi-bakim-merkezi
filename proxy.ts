...
function roleAllowed(pathname: string, role: Role) {
    ...
    if (role === 'goruntuleyici')
        return pathname === '/' || pathname.startsWith('/arizalar') || pathname.startsWith('/raporlar') || pathname.startsWith('/motorlar');
    if (role === 'teknisyen')
        return pathname === '/' || pathname.startsWith('/teknisyen') || pathname.startsWith('/arizalar') || pathname.startsWith('/bildirimler');
    if (role === 'operator')
        return pathname === '/' || pathname.startsWith('/arizalar') || pathname.startsWith('/bildirimler');
    return false;
}
export async function proxy(request: NextRequest) {
    ...
    try {
        ...
        if (!['yonetici', 'teknisyen', 'operator', 'goruntuleyici'].includes(role))
