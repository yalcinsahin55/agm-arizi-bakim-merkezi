...
if (Array.isArray(src.motors)) {
    for (const value of src.motors) {
        ...
        await d.collection('motors').updateOne({ legacyKey }, { $set: { name, legacyKey, active: value?.active !== false, source: String(value?.source || 'agm-bakim-merkezi'), currentHours: Number(value?.hours ?? value?.currentHours ?? 0) || 0, currentLoad: Number(value?.load ?? value?.currentLoad ?? 0) || 0, sourceUpdateDate: src.sourceUpdateDate || src.updateDate || null, updatedAt: new Date() }, $setOnInsert: { _id: randomUUID(), hours: Number(value?.hours) || 0, load: Number(value?.load) || 0, createdAt: new Date(), importedAt: new Date() } }, { upsert: true });
        imported++;
    }
}
else {
