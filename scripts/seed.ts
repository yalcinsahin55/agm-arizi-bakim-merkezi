...
if (process.env.NODE_ENV === 'production' && !process.env.SEED_DEMO_PASSWORD)
    throw new Error('Production seed için SEED_DEMO_PASSWORD zorunludur');
...
for (const m of motorData.motors) {
    await d.collection('motors').updateOne({ legacyKey: m.legacyKey }, { $set: { name: m.name, legacyKey: m.legacyKey, equipmentType: 'motor', active: true, source: m.source, updatedAt: new Date() }, $setOnInsert: { _id: randomUUID(), hours: Number(m.hours) || 0, load: Number(m.load) || 0, createdAt: new Date(), importedAt: new Date() } }, { upsert: true });
}
