...
for (const m of motorData.motors) {
    await d.collection('motors').updateOne({ legacyKey: m.legacyKey }, { $set: { name: m.name, legacyKey: m.legacyKey, active: true, source: m.source, updatedAt: new Date() }, $setOnInsert: { _id: randomUUID(), hours: Number(m.hours) || 0, load: Number(m.load) || 0, createdAt: new Date(), importedAt: new Date() } }, { upsert: true });
}
